...
\begin{itemize}
  \item \texttt{symptoms.json} -- a
    ...
    Each profile carries (a) a
    \texttt{VisualSymptom} block (plant parts,
    color, shape, margin, texture, sporulation,
    distinctive signs, progression, confusion
    diseases, free-form notes); (b) per-state
    and per-AEZ observation counts; (c) a list
    of reference IDs; (d) an auto-built
    \texttt{reobservation\_prompt} composed
    from the populated visual fields; and (e)
    an optional \texttt{SwarmObservations}
...
For each of the $484$ profiles,
\texttt{scripts/seed\_pathome\_with\_claude.py}
issues a single subprocess call:
\begin{verbatim}
claude -p "<schema-fixed prompt>"
       --output-format json
       ...
       --model sonnet
\end{verbatim}
The prompt fixes a JSON schema enumerating the
\texttt{VisualSymptom} fields and instructs the
model to return empty arrays/strings rather
than guessing for diseases it is uncertain
about. The response is parsed via the
envelope's \texttt{result} key, code-fence
stripped, validated against the schema, and
written incrementally into the seed file
so that walltime kills resume cleanly.
Failures (timeouts, malformed JSON, schema
violations) are logged to \texttt{failed.jsonl}
and resubmittable via \texttt{--retry-failed}.
At four parallel workers and $\sim$$7$\,s per
profile (Sonnet), the full $484$-class seed
lands in well under an hour and costs a fraction
of a single curator-day. We explicitly do
\emph{not} rely on the seed being expert-grade
on every profile; it is the input to the
trace-driven enhancement pass, not the
endpoint.
...
\end{itemize}
The visual block from Phase~0 is preserved
verbatim. The geo prior (Eq.~\ref{eq:density})
reads its counts directly off
\texttt{state\_counts}---there is no separate
...
\texttt{SymptomProfile}; the visual content
from Phase~0 is left untouched. The output
is a new on-disk DB,
